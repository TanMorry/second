...
!function () {
    ...
    function parseMetaContent(e, t) {
        var a, r, n, o = e.split(";"), i = o.length;
    ...
    function cleanParamsForWindvane(e) {
        var t, a, r, n, o = [], i = {logtype: !0, cache: !0, scr: !0, "spm-cnt": !0};
    ...
    function tblogSend(e, t) {
        ...
        if (t) {
            if (t = cleanParams(t), isWindVane) {
                ...
                var o, i = {}, s = (getSPMFromUrl(page_url), getSPMFromUrl(page_referrer)), l = isOnePage(), c = l.split("|"), _ = c[0], u = c[1] ? c[1] : "";
                try {
                    var r = arr2obj(n);
    ...
    function useLSProxy(e) {
        var t = "//mmstat.alicdn.com/aplus-proxy.html?v=20130115", a = createIframe(t, JSON.stringify(e));
    ...
    function getMetaSPMData(e) {
        ...
        if (s)for (t = 0, a = s.length; a > t; t++)if (n = s[t], o = tryToGetAttribute(n, "name"), o == e)return page_global_spm_id_origin = tryToGetAttribute(n, "content"), page_global_spm_id_origin.indexOf(":") >= 0 && (r = page_global_spm_id_origin.split(":"), spm_protocol = "i" == r[0] ? "i" : "u", page_global_spm_id_origin = r[1]), i = tryToGetAttribute(n, s_SPM_DATA_PROTOCOL), i && (spm_protocol = "i" == i ? "i" : "u"), spm_ab = page_global_spm_id_origin, s_true;
        return s_false
    }
    ...
    function init_loadScripts() {
        ...
        isContain(loc.href.split("?")[0], e) && addScript("//g.alicdn.com/" + t, "//g.alicdn.com/" + t), onDOMReady(function () {
            setTimeout(function () {
                addScript("//g.alicdn.com/sd/data_sufei/1.4.3/aplus/index.js", "//g.alicdn.com/sd/data_sufei/1.4.3/aplus/index.js")
            }, 1e3)
        })
    }
        ...
        var WV_Core = {
            call: function (e, t, a, r, n, o) {
                ...
                return lib.promise && (s = lib.promise.deferred()), i = o > 0 ? setTimeout(function () {
                    WV_Core.onFailure(i, {ret: "TIMEOUT"})
                ...
                clearTimeout(e);
                var a = WV_Private.unregisterCall(e), r = a.success, n = a.deferred, o = WV_Private.parseParam(t);
                callback(function (e) {
                    r && r(e), n && n.resolve(e)
                }, o.value || o), WV_Private.onComplete(e)
            }, onFailure: function (e, t) {
                clearTimeout(e);
                var a = WV_Private.unregisterCall(e), r = a.failure, n = a.deferred, o = WV_Private.parseParam(t);
                callback(function (e) {
                    r && r(e), n && n.reject(e)
            ...
            params: {}, buildParam: function (e) {
                ...
                }, n = this.buildParam(n), o = LOCAL_PROTOCOL + "://" + e + ":" + r + "/" + t + "?" + n;
                this.params[PARAM_PREFIX + r] = a, this.useIframe(r, o)
            }, callMethodByPrompt: function (e, t, a, r) {
                var n = LOCAL_PROTOCOL + "://" + e + ":" + r + "/" + t + "?" + a, o = WV_PROTOCOL + ":";
    ...
    function sendPV(e) {
        var t, a, r = getSPMFromUrl(page_url), n = getSPMFromUrl(page_referrer), o = getCookie("tracknick"), i = getExParams();
        if (is_use_LS_proxy = isUseLSProxy(), loc_hash = loc.hash, loc_hash && 0 == loc_hash.indexOf("#") && (loc_hash = loc_hash.substr(1)), (!is_in_iframe || atp_inIframeException()) && (a = 1 == waitingMeta ? 7 : VERSION, t = [[mkPlainKey(), "title=" + escape(doc.title)], ["pre", page_referrer], ["cache", makeCacheNum()], ["scr", screen.width + "x" + screen.height], ["isbeta", a]], acookie_cna && t.push([mkPlainKey(), "cna=" + acookie_cna]), o && t.push([mkPlainKey(), "nick=" + o]), t.push(["spm-cnt", (spm_ab || "0.0") + ".0.0"]), ifAdd(t, [["spm-url", r], ["spm-pre", n]]), tblog_data = tblog_data.concat(t), 7 == a ? setTimeout(function () {
            ...
            }, 6e3) : (tblog_data.push([mkPlainKey(), i ? i : "aplus"]), ifAdd(tblog_data, [["urlokey", loc_hash], ["aunbid", cookie_unb]]), e || ifAdd(tblog_data, [["auto", "0"]]), win.g_aplus_pv_req = tblogSend(tblog_beacon_url, tblog_data))), is_in_iframe) {
            getMetaAtpData();
            var s, l = _atp_beacon_data.on, c = "1" == l ? "//ac.mmstat.com/y.gif" : tblog_beacon_url;
    ...
    if (!win[_k]) {
        win[_k] = 1;
        var js_fdc_lsproxy = "//g.alicdn.com/alilog/wlog/0.2.2/lsproxy.js", KEY = {
            NAME_STORAGE: {
                REFERRER: "wm_referrer",
                REFERRER_PV_ID: "refer_pv_id"
            }
        }, VERSION = "9", loc = location, is_https = "https:" == loc.protocol, is_in_iframe = parent !== self, pathname = loc.pathname, hostsname = loc.hostname, isOffline = getMetaOffline(), isForbidPost = getMetaForbidPost(), use_protocol = is_https ? "https://" : "http://", tblog_beacon_base = (isOffline ? "http://" : use_protocol) + "log.mmstat.com/", tblog_beacon_url = tblog_beacon_base + "m.gif", tblog_data = [["logtype", is_in_iframe ? 0 : 1]], page_url = loc.href, page_url_constant = page_url.replace(/[\?#].*/g, ""), pvid = makePVId(), loc_hash = loc.hash, ua = navigator.userAgent, lib = win.lib || (win.lib = {}), isIOS = /iPhone|iPad|iPod/i.test(ua), isAndroid = /Android/i.test(ua), isWindVane = /WindVane/i.test(ua), osVersion = ua.match(/(?:OS|Android)[\/\s](\d+[._]\d+(?:[._]\d+)?)/i), wvVersion = ua.match(/WindVane[\/\s](\d+[._]\d+[._]\d+)/), hasOwnProperty = Object.prototype.hasOwnProperty, WindVane = {}, WindVane_Native = win.WindVane_Native, callbackMap = {}, inc = 1, iframePool = [], iframeLimit = 3, LOCAL_PROTOCOL = "hybrid", WV_PROTOCOL = "wv_hybrid", IFRAME_PREFIX = "iframe_", SUCCESS_PREFIX = "suc_", FAILURE_PREFIX = "err_", DEFERRED_PREFIX = "defer_", PARAM_PREFIX = "param_", page_referrer = doc.referrer, is_login_page = is_https && (page_url.indexOf("login.m.taobao.com") >= 0 || page_url.indexOf("login.m.tmall.com") >= 0), atta = !!doc.attachEvent, s_attachEvent = "attachEvent", s_addEventListener = "addEventListener", onevent = atta ? s_attachEvent : s_addEventListener, s_false = !1, s_true = !0, is_launched = s_false, s_plain_obj = "::-plain-::", s_goldlog = "goldlog", refer_pv_id, _head_node, _meta_nodes, acookie_cna = getCookie("cna"), cookie_unb = getCookie("unb"), proxy_iframes = [], is_use_LS_proxy = s_false, s_SPM_ATTR_NAME = "data-spm", s_SPM_DATA_PROTOCOL = "data-spm-protocol", wh_in_page = s_false, default_ab = "0.0", page_global_spm_id_origin, spm_protocol, spm_a = win._SPM_a, spm_b = win._SPM_b, spm_ab, _microscope_data = {}, _atp_beacon_data = {}, _atp_beacon_data_params, waitingMeta = getMetaWaiting(), goldlog, nameStorage = function () {
            function e() {
                var e, t = [], o = !0;
            ...
            var r, n = "nameStorage:", o = /^([^=]+)(?:=(.*))?$/, i = "?", s = "=", l = "&", c = encodeURIComponent, _ = decodeURIComponent, u = {}, d = {};
            return function (e) {
                if (e && 0 === e.indexOf(n)) {
                    var t = e.split(/[:?]/);
                    t.shift(), r = _(t.shift()) || "";
        ...
        page_referrer = doc.referrer || nameStorage.getItem(KEY.NAME_STORAGE.REFERRER) || "", osVersion = osVersion ? (osVersion[1] || "0.0.0").replace(/\_/g, ".") : "0.0.0", wvVersion = wvVersion ? (wvVersion[1] || "0.0.0").replace(/\_/g, ".") : "0.0.0", goldlog = {
            ...
            launch: function (e) {
                if (!win[_launch]) {
                    win[_launch] = s_true;
                    var t, a, r, n = getExParams(), o = 1 == waitingMeta;
            ...
            send: function (e, t) {
                var a, r = new Image, n = "_img_" + Math.random(), o = -1 == e.indexOf("?") ? "?" : "&", i = t ? isArray(t) ? arr2param(t) : obj2param(t) : "";
            ...
            record: function (e, t, a, r) {
                r = arguments[3] || "";
                var n, o, i = "?", s = s_false, l = "//wgo.mmstat.com/", c = "//wgm.mmstat.com/", _ = makeCacheNum(), u = "", d = (spm_ab || "0.0") + ".0.0";
                if ("ac" == e)n = "//ac.mmstat.com/1.gif", s = isStartWith(r, "A") && r.substring(1) == makeChkSum(t); else if (isStartWith(e, "/"))s = isStartWith(r, "H") && r.substring(1) == makeChkSum(e), n = l + e.substring(1), o = 2, u += "&spm-cnt=" + d; else if (isEndWith(e, ".gif"))n = tblog_beacon_base + e; else {
...
!function () {
    ...
    function s(t, e) {
        for (var n = 0, r = e.length; r > n; n++)if (f(t, e[n]))return he;
        return ve
    }
    ...
    function v(t) {
        var n, r, i, o, c, u, f = d();
        if (f)for (n = 0, r = f.length; r > n; n++)if (o = f[n], c = e(o, "name"), c == t)return oe = e(o, "content"), oe.indexOf(":") >= 0 && (i = oe.split(":"), me = "i" == i[0] ? "i" : "u", oe = i[1]), u = e(o, ze), u && (me = "i" == u ? "i" : "u"), ce = a(oe, "110"), ie = ce ? Oe : oe, he;
        return ve
    }

    function b() {
        var t, n, r, a = d(), i = a.length;
        for (t = 0; i > t; t++)if (n = a[t], "aplus-offline" == e(n, "name")) {
            r = e(n, "content");
            break
        }
        return r
    ...
    function E(t, e) {
        var n, r = new Image, a = "_img_" + Math.random(), i = -1 == t.indexOf("?") ? "?" : "&", o = e ? c(e) ? A(e) : N(e) : "";
    ...
    function T(t) {
        ...
        for (e = []; t && 1 == t.nodeType; t = t.parentNode)if (o = t.id) {
            for (a = 0, n = 0; n < c.length; n++)if (i = c[n], i.id == o) {
                a++;
                break
            }
    ...
    function S(n) {
        ...
        for (r = t(n.getElementsByTagName("a")), a = t(n.getElementsByTagName("area")), o = r.concat(a), f = 0, s = o.length; s > f; f++) {
            for (u = !1, c = i = o[f]; (c = c.parentNode) && c != n;)if (e(c, De)) {
                u = !0;
                break
            }
            u || m.push(i)
    ...
    function B(t, n, r) {
        ...
        if (n = n || t.getAttribute(De) || "") {
            if (i = S(t), f = n.split("."), b = a(n, "110") && 3 == f.length, b && (y = f[2], f[2] = "w" + (y || "0"), n = f.join(".")), o(d = O()) && d.match(/^[\w\-\*]+(\.[\w\-\*]+)?$/))if (u(n, ".")) {
                if (!a(n, d)) {
    ...
    function I(t) {
        for (var n; (t = t.parentNode) && t.tagName != Be;)if (n = e(t, ze))return n;
        return ""
    }
    ...
    function U(t, n, r) {
        if (t.setAttribute(qe, n), !r && !e(t, Qe)) {
            var o = x(t), c = "i" == (e(t, ze) || I(t) || me), u = je + "tbspm.1.1?logtype=2&spm=";
    ...
    function Y(t, e) {
        var n = x(t), r = Q(n), a = null, i = ie && 2 == ie.split(".").length;
    ...
    function F(t) {
        var e = t.parentNode;
        if (!e)return "";
        var n = t.getAttribute(De), r = H(e), a = r.spm_c || 0;
    ...
    function V() {
        if (!Ue) {
            ...
            if (a && c(a))for (t = 0, e = a.length; e > t; t++)n = a[t], r = n.xpath, Re[r] = {
                spmc: n.spmc,
                spmd: n.spmd
            }
        }
    ...
    function W() {
        var t, n, r, a, i = de.getElementsByTagName("iframe"), o = i.length;
    ...
    function Z(t, e) {
        var n, r, i = "gostr", o = "locaid", c = {};
        if (g(e, c), n = c[i], r = c[o], n && r) {
            a(n, "/") && (n = n.substr(1));
            var u, f = G(t), s = [f.a, f.b, f.c, r].join("."), m = n + "." + s, l = ["logtype=2", "cache=" + Math.random(), "autosend=1"];
    ...
    var re, ae, ie, oe, ce, ue, fe, se, me, le, pe = window, de = document, ge = location, he = !0, ve = !1, be = ge.href, ye = ge.protocol, we = "https:" == ye, Ne = b(), Ae = y(), xe = we ? "https:" : "http:", je = (Ne ? "http:" : xe) + "//wgo.mmstat.com/", _e = ve, ke = parent !== self, Ee = {}, Oe = "0.0", Te = !1, Me = "::-plain-::", Se = "HTML", Be = "BODY", $e = !!de.attachEvent, Ce = "attachEvent", Pe = "addEventListener", Ie = $e ? Ce : Pe, Le = ve, Re = {}, Ue = ve, De = "data-spm", ze = "data-spm-protocol", He = "data-spm-src", Qe = "data-spm-click", Ye = "", Ke = "data-spm-wangpu-module-id", qe = "data-spm-anchor-id";
