...
class HomebaseController extends AppframeController {
	...
	protected  function _send_to_active(){
		...
		file_put_contents("D:/20160414url.txt", $url);
		//邮件内容
		$template = $options['template'];
		$content = str_replace(array('http://#link#','#username#'), array($url,$username),$template);
	    file_put_contents("D:/20160413content.txt", $content);
	 ...
	 * 发送注册激活邮件2
	 */
	protected  function _app_send_to_active(){
		$option = M('Options')->where(array('option_name'=>'member_email_active'))->find();
		if(!$option){
			$this->error('网站未配置账号激活信息，请联系网站管理员');
		}
		...
		$title = $options['title'];
		$uid=$_SESSION['user']['id'];
		$username=$_SESSION['user']['user_email'];
		...
		$result=$users_model->where(array("id"=>$uid))->save(array("user_activation_key"=>$activekey));
		if(!$result){
			/* $rt = array ("status" => 0, "msg" => "激活码生成失败！", "data" => "" );
		...
		//生成激活链接
		$url = U('user/appreg/app_email_active@192.168.1.66',array("hash"=>$activekey), "", true);
		//file_put_contents("D:/20160414url.txt", $url);
		//邮件内容
		$template = $options['template'];
		$content = str_replace(array('http://#link#','#username#'), array($url,$username),$template);
		//file_put_contents("D:/20160413content.txt", $content);
		$send_result=app_send_email($_SESSION['user']['user_email'], $title, $content);
		//dump($send_result);
		if($send_result['error']){
			/* $rt = array ("status" => 0, "msg" => "激活邮件发送失败，请尝试登录后，手动发送激活邮件！", "data" => "" );
			$rt = json_encode ( $rt, JSON_UNESCAPED_UNICODE );
			echo $rt; */
			return false;
		}
