...
//    echo "页面分页跳转地址(getPage方法)：".$module_name."<br>";
    ...
    $Page->setConfig('header','<span class="bottom_skip"><span id="jump">&nbsp;跳转到&nbsp;<input type="tel" size="4" class="skiptext" style="width:30px;border:1px solid #ccc;" id="z"/>&nbsp;页&nbsp;<a class="tiaozhuan" href="' . $module_name . '&p=" id="test" >跳转</a></span>&nbsp;<span>当前第%NOW_PAGE%页</span>&nbsp;<span title="%TOTAL_PAGE%" class="all">共&nbsp;<b id="totalPage">%TOTAL_PAGE%</b>&nbsp;页</span></span>');
...
{
    $pattern = '#<img src="(?!http:\/\/img.baidu)([^"]+)"#';
    foreach($list as $key=>$val)
    {
        $new = htmlspecialchars_decode($val['content']);
...
/*
 *后台将文章中的图片变成‘【图】’，文字取前50个字（去掉标签，提取前五十字符）
 * @param       $content   内容
 */
function handleReply($content)
{
    ...
    if(strlen($contents) > 150)//通过统计字节数的长度 GBK/GB2312下，中文字符占2个字节，而在UTF-8下，中文字符占3个字节
    {
        $shortText = mb_substr($contents,0,50,'utf-8').'...';
    }
    else
    {
        $shortText = mb_substr($contents,0,50,'utf-8');
    }
    $pattern ='#<img src="(?!http:\/\/img.baidu)([^"]+)"#';//将图片转换为‘【图】’
    $new = htmlspecialchars_decode($content, ENT_QUOTES );
    preg_match_all($pattern,$new,$match);
    $matchNum = count($match[0]);
    $str="";
    if($matchNum>0)
    {
        if($matchNum>3)
