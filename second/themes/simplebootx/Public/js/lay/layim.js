...
function reciveMsg(data)
{
	/**var description = ""; 
	 ...
	 console.log("senduser:" + data.getSenderUserId());**/
	 $('#ulist'+data.getSenderUserId()).append("new");
	 $.ajax({
		    type: 'get',
		    url: '/tongrenmiao/?g=exhibition&m=index&a=getLeastSend',
		    data: 'sendid='+data.getSenderUserId(),
		    success: function(result){
		    	console.log("result:" + result);
		    	//result = result+"";
		    	//var end = result.length - 2;
		    	//alert(end);
		    	//result = result.substr(1,end);
		    	//alert(result);
		    	 result = eval(result);
		    	 if($('#layim_areaone'+data.getSenderUserId()).length>0)
		    	 {
		    		$('#ulist'+data.getSenderUserId()).html("");
		    		$('#layim_areaone'+data.getSenderUserId()).append('<li class="layim_chatme"><div class="layim_chatuser">'
		    				+'<img src="'+result[0].avatar+'"/>'
		    				+'<span class="layim_chatname">'+result[0].user_nicename+'</span>'
		    				+'<span class="layim_chattime">'+result[0].sendtime+'</span></div><div class="layim_chatsay">'+result[0].content+'<em class="layim_zero"></em></div></li>');

			    	$('#layim_areaone'+data.getSenderUserId()).scrollTop($('#layim_areaone'+data.getSenderUserId())[0].scrollHeight);
		    	 }		    			    
		    }
...
function getMsgs(chatid)
{
	$.ajax({
	    ...
	    url: '/tongrenmiao/?g=exhibition&m=index&a=getMsgs',

	    data: 'chatid='+chatid,

	    success: function(data){
	    	$('#layim_areaone'+chatid).html('');
	    	console.log("data:" +data);
	    	data = eval(data);
	    	for(var i=0;i<data.length;i++)
	    	{
	    		console.log("data:" +data[i]);
	    		console.log("id:" + chatid+'--'+data[i].sendid);
	    		if(chatid != data[i].sendid)
	    		{
	    			$('#layim_areaone'+chatid).append('<li class="layim_chateme"><div class="layim_chatuser">'
		    				+'<span class="layim_chattime">'+data[i].sendtime+'</span><span class="layim_chatname">'+data[i].user_nicename+'</span>'+
		    				'<img src="'+data[i].avatar+'"/></div><div class="layim_chatsay">'+data[i].content+'<em class="layim_zero"></em></div></li>');
	    		}
	    		else
	    		{
	    			$('#layim_areaone'+chatid).append('<li class="layim_chatme"><div class="layim_chatuser">'
	    					+'<img src="'+data[i].avatar+'"/>'
		    				+'<span class="layim_chatname">'+data[i].user_nicename+'</span>'
		    				+'<span class="layim_chattime">'+data[i].sendtime+'</span></div><div class="layim_chatsay">'+data[i].content+'<em class="layim_zero"></em></div></li>');

	    		}	    	
	    	}
	    	$('#layim_areaone'+chatid).scrollTop($('#layim_areaone'+chatid)[0].scrollHeight);
...
function chat(id,name,face)
{
	param = {id:id,type:"one",name:name,face:"/tongrenmiao/themes/simplebootx//Public/images/headicon.png",href:"#"}
	 myim.popchat(param);
}
...
var config = {
    ...
    right: -232,
    api: {
        friend: 'http://127.0.0.1/tongrenmiao/friend.json', //好友列表接口
        group: 'http://127.0.0.1/tongrenmiao/group.json', //群组列表接口 
        chatlog: '/tongrenmiao/?g=exhibition&m=index&a=getChatUsers', //聊天记录接口
        groups: 'http://127.0.0.1/tongrenmiao/groups.json', //群组成员接口
        sendurl: '' //发送消息接口
    },
    user: getUser(),
...
xxim.transmit = function(){
    ...
    log.send = function(){
        ...
        if(data.content.replace(/\s/g, '') === ''){
            ...
            console.log("Login successfully." + keys);
            //聊天模版
            ...
            sendMsg(xxim.nowchat.id, data.content);
    ...
    config.json(api[index], {}, function(datas){
        if(datas.status === 1){
            ...
            if(myflen > 0){
                ...
                for(; i < myflen; i++){
                    str += '<li data-id="'+ datas.data[i].sendid +'" class="xxim_childnode" type="one"><img src="'+ datas.data[i].avatar +'"  class="xxim_oneface"><span  class="xxim_onename">'+ datas.data[i].user_nicename +'</span><span id="ulist'+ datas.data[i].sendid +'"></span><em class="xxim_time">'+ datas.data[i].sendtime +'</em></li>'; 
                }
                str += '</ul></li>';
                ...
                myf.html('<li class="xxim_errormsg"><a href="#">刷新一下试试</a></li>');
            }
...
xxim.view = (function(){
    var xximNode = xxim.layimNode = $('<div id="xximmm" class="xxim_main">'
            ...
            +'  <div class="xxim_tabs" id="xxim_tabs"><span class="xxim_latechat"  title="最近聊天"><i></i></span></div>'
